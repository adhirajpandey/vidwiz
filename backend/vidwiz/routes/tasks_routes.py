...
def get_metadata_task():
    ...
    try:
        ...
        logger.exception(f"Unexpected error in get_metadata_task: {e}")
        return jsonify({"error": "Internal Server Error"}), 500
...
def submit_metadata_result():
    ...
    try:
        ...
        try:
            result_data = MetadataResult(**data)
        except ValidationError as e:
        ...
        db.session.rollback()
        logger.exception(f"Unexpected error in submit_metadata_result: {e}")
        return jsonify({"error": "Internal Server Error"}), 500
